...
import DestinationHomeLoading from "./DestinationHomeLoading";
const Destinations = () => {
  const text1 = "الرحلات السياحية";
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-12 gap-4 lg:gap-6 px-3 xl:px-0">
            {locations?.map((location) => (
              <Link
                href={`/tour-listing?destination=${location.name?.replaceAll(
                  " ",
                  "-"
                )}`}
                key={location.id}
